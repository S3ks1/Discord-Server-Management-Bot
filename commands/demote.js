...
module.exports.run = async (client, message, msgargs, command) => {
    ...
    if(message.guild.members.cache.get(message.author.id).roles.highest.position <= role.position){
        let embed = new Discord.MessageEmbed().setDescription(`:warning: You can't give this role as it is higher than your highest role or your highest role`).setColor(message.embedColor).setTimestamp()
        message.channel.send({
            embed
        });
        return;
    ...
    Guild.findOne({ guildId: message.guild.id} ).then((res) => {
        ...
        if(role.name == res.defaultRole){
            user.roles.cache.forEach(r=>{
                ...
                user.roles.add(message.guild.roles.cache.find(r=>r.name === res.defaultRole))
                let embed = new Discord.MessageEmbed()
                .setDescription(`:ok_hand: Demoted ${user.user.tag} to ${role}`)
                .setTimestamp()
                ...
                .setTitle(`${message.guild.name} | Demotion`)
                .setDescription(`${user} has been Demoted to ${role} by ${message.author}`)
                .setFooter(`Demoted by ${message.author.username}`, message.author.displayAvatarURL({
                    dynamic: true
                }))
                .setColor(message.embedColor)
                .setTimestamp()
        ...
        else if(!user.roles.cache.find(r => r.name == role.name)){
            user.roles.cache.forEach(r=>{
                if(r.name !== "@everyone"){
                    user.roles.remove(r)
                }
            ...
            let embed = new Discord.MessageEmbed()
            .setDescription(`:warning: The provided user has this role already`)
            .setTimestamp()
            .setColor(message.embedColor)
            message.channel.send({
                embed
...
module.exports.help = {
    ...
    usage: `demote <user> <role name or id>`,
    aliases: ["demo", "dem"],
